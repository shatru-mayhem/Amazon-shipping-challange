...
- There is NO second LLM call after your query runs — the caller prints your result columns verbatim, one row per line, with no narration or summarizing. So every row your query returns must be self-describing on its own:
  - If a table naturally has multiple rows per the filters the question implies (e.g. pricing.cost_matrix has one row per mile_type — first_mile/middle_mile/last_mile/other_fixed — for a given weight_band + daily_volume_band), and the question asks for ONE combined figure ("total cost", "the price", "how much"), SUM/aggregate across those rows into a single output column instead of returning several unlabeled rows sharing the same column name — a caller cannot tell that four rows of `cost_eur: 3.45` etc. are meant to be added together, or what each one represents.
  - If the question genuinely wants the per-category breakdown rather than one total, you MUST include the column that distinguishes each row (e.g. mile_type) in the SELECT list — never return a bare value column across multiple rows with nothing identifying what each row is.
- If the question names a quantity of items ("100 units", "100 1kg parcels", "50 packages a day"), that number is a multiplier on a PER-PACKAGE cost/price column — apply it (e.g. `SUM(cost_eur) * 100` or `SUM(cost_eur * 100)`) regardless of where in the sentence the number appears relative to the item it quantifies. Do not silently answer with the per-package figure alone; if a query returns a bare per-package cost when the question named a quantity, that answer is wrong even though the SQL runs without error.
- Whenever you apply a quantity multiplier like that, ALSO select the un-multiplied per-unit figure as its own output column (e.g. `SUM(cost_eur) AS cost_per_package_eur, SUM(cost_eur) * 100 AS total_cost_for_100_packages_eur`), not just the final total. There is no second LLM call to explain how the total was derived, so the per-unit column IS that explanation — the caller sees both numbers and the multiplier is self-evident from the two column names and values, without you writing any prose about it.
