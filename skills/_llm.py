...
a personal namespace, and it has no embedding models at all:
  ...
  - EMBED_MODEL   (nomic-embed-text) -> embed() / embed_batch()
    ...
    exist, and a real calibration against this project's own tender data
    showed Gemini's gemini-embedding-001 (tried plain, SEMANTIC_SIMILARITY,
    and RETRIEVAL_QUERY/RETRIEVAL_DOCUMENT task types) discriminating
    noticeably worse than nomic-embed-text on this exact short-catalog-
    description-vs-long-passage matching task — an irrelevant chunk
    routinely outscored the genuinely relevant one. Not a threshold issue,
    a real per-model quality difference, so embeddings stay on nomic.
